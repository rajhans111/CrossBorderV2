      ...
      <QueryState isLoading={isLoading} error={error} onRetry={refetch}>
        {data && (
          <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
            ...
            <Card>
              <h2 className="mb-3 font-semibold text-gray-900">Compliance artefacts</h2>
              {data.complianceArtefacts.length === 0 ? (
                <p className="text-sm text-gray-400">No filings yet.</p>
              ) : (
                <ul className="space-y-2">
                  {data.complianceArtefacts.map((artefact) => (
                    <li
                      key={artefact.id}
                      className="flex items-center justify-between rounded-lg border border-gray-100 px-4 py-2 text-sm"
                    >
                      <div>
                        <p className="font-medium text-gray-900">{artefact.type}</p>
                        <p className="text-xs text-gray-500">
                          {data.orders.find((o) => o.id === artefact.orderId)?.reference ?? artefact.orderId}
                        </p>
                      </div>
                      <span
